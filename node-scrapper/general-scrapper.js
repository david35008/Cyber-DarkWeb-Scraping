...
module.exports = async function main(ymlFile, pageNum) {

    const config = yaml.safeLoad(
        fs.readFileSync(`./configurations/${ymlFile}.yml`, "utf8")
    );
    ...
    try {
        ...
        if (config["need-timeout"]) {
            ...
            await Promise.all(
                links.map(async (link, index) => {
                    ...
                    let elemDate = $$(config["tasks"]["get-pastes-dates"]["html-tag"]).first()
                    if (config["tasks"]["get-pastes-dates"]["is-in-string"]) {
                        const dateFull = $$(elemDate).text().trim()
                        let date;
                        if (
        ...
        const combinedData = titles
            .map((item, index) => {
                return {
                    title: item,
                    content: contents[index].slice(0,65000),
                    date: dates[index],
